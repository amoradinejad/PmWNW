## This module computes the EH fit (astro-ph/9709112) for wiggle and no-wiggle split of the matter power spectrum 
...
# s      = 2./(3.*keq)*np.sqrt(6./Req)*np.log((np.sqrt(1.+Rd)+np.sqrt(Rd+Req))/(1.+np.sqrt(Req)))   ### The exact equation for the sound speed in Eq. 2 of EH that I used initially  
 ###If I use this compared to the next equation, I get some reisual at the k~0.01, so I am using the next one
s = 44.5 * np.log(9.83/om0h2)/np.sqrt(1.+10.*ombh2**(3./4.))    ## approximate sound speed given in Eq. (26) of EH, used by Martin 

# ###parmas for small-scale limit
def qq(k):
...
def T(k):
    out = ombh2/om0h2 * Tb(k) + omch2/om0h2 * Tc(k)
    return out
