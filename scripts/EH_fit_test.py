...
class EHFit:
    ...
    def Tt0(self,k, x1, x2):  ##x1 = alphac, x2 = betac
        out = np.log(math.e+1.8*x2*qq(self,k))/(np.log(math.e+1.8*x2*self.qq(k))+self.C(k, x1)*self.qq(k)**2.)
        return out 

    def Tc(self,k): 
        out = self.f(k)*self.Tt0(k,1.,self.betac) + (1.-self.f(k)) *self.Tt0(k,alphac,betac)                                            
        return out
    ...
    def T(self,k):
        s   = cs()
        out = self.ombh2/self.om0h2 * self.Tb(k,s) + self.omch2/self.om0h2 * self.Tc(k)
        return out
    ...
    def q(self,k):
        out = k/h * self.theta**2./self.Gamma(k)                                        
        return out
    ...
    def EH_PS_w(self,k,k0,p0):
        kh   = self.h*k
        kh0  = self.h*k0
        norm = p0/(k0**self.ns*self.T(kh0)**2.)
        out  = k**self.ns * self.T(kh)**2. * norm
        return eh_w

    def EH_PS_nw(self,k,k0,p0):
        kh   = self.h*k
        kh0  = self.h*k0
        norm = p0/(k0**self.ns*self.T(kh0)**2.)
        out  = k**self.ns * self.T0(kh)**2. * norm
        return eh_nw
